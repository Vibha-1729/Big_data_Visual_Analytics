...
with open("project661\india.json", encoding="utf-8") as f:
...
state_df_pct = pd.read_excel(
    r"C:\Users\aadya\OneDrive\Desktop\project661\project661\statewise_aggregated_data_percentages.xlsx"
)

def state_value(state_name: str, col: str):
    """Return the value for a state & column; None if missing."""
    row = state_df_pct[state_df_pct["State name"].str.lower()
                       == state_name.lower()]
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Graph(id='india-map', style={'height': '85vh'}),
    ], style={
        'width': '50%', 'display': 'inline-block', 'verticalAlign': 'top',
        'padding': '10px', 'backgroundColor': 'white', 'position': 'relative'
    }),  # ← comma here
    ...
    html.Div([
        ...
        dbc.Row([
            dbc.Col(dbc.Card([
                dbc.CardHeader("District Value"),
                dbc.CardBody(html.H5(id="kpi-district"))
            ], color="primary", inverse=True), width=4),

            dbc.Col(dbc.Card([
                dbc.CardHeader("State Average"),
                dbc.CardBody(html.H5(id="kpi-state"))
            ], color="info", inverse=True), width=4),

            dbc.Col(dbc.Card([
                dbc.CardHeader("Δ from State"),
                dbc.CardBody(html.H5(id="kpi-delta"))
            ], color="warning", inverse=True), width=4),
        ], className="mb-3"),

        # district breakdown bar
        dcc.Graph(id='district-bar', style={'height': '32vh'}),
        dcc.Dropdown(id='subcategory-dropdown', placeholder="Select subcategory", style={'width': '100%'}),

        # box-whisker + district map
        html.Div([
            dcc.Graph(id='box-whisker', style={'width': '48%', 'display': 'inline-block', 'height': '28vh'}),
            dcc.Graph(id='state-district-map', style={'width': '48%', 'display': 'inline-block', 'height': '28vh'}),
        ], style={'display': 'flex', 'justifyContent': 'space-between'}),

        # correlation selector + pies
        html.Div([
            html.Div([
                html.Div("Correlation", style={'fontWeight': 'bold'}),
                dcc.Dropdown(id='correlation-attribute-dropdown', options=[{"label": k, "value": k} for k in ATTRIBUTE_MAP], placeholder="Select attribute")
            ], style={'width': '28%', 'display': 'inline-block', 'padding': '10px', 'backgroundColor': '#f8f9fa'}),

            html.Div([
                dcc.Graph(id='top5-pie', style={'width': '48%', 'display': 'inline-block', 'height': '24vh'}),
                dcc.Graph(id='bottom5-pie', style={'width': '48%', 'display': 'inline-block', 'height': '24vh'}),
            ], style={'width': '70%', 'display': 'inline-block', 'verticalAlign': 'top'}),
        ], style={'display': 'flex', 'justifyContent': 'space-between', 'marginTop': '10px'}),

        # correlation scatter + description
        html.Div([
            html.Div(id='correlation-description', style={'flex': '1', 'padding': '10px'}),
            dcc.Graph(id='correlation-scatter', style={'height': '28vh', 'width': '100%'}),
        ], style={'marginTop': '16px'}),
    ], style={
        'width': '48%', 'display': 'inline-block', 'verticalAlign': 'top',
        'position': 'absolute', 'top': '0', 'right': '0',
        'padding': '10px', 'backgroundColor': 'white', 'boxShadow': '0 2px 8px rgba(0,0,0,0.07)'
    }),
...
@app.callback(
    ...
    [
        ...
        Input("subcategory-dropdown", "value"),       # NEW
        Input("attribute-category-dropdown", "value") # NEW
    ],
)

def draw_india_map(selected_state, selected_subcat, selected_category):
    """
    Colour every state by the chosen metric (percentage column if present).
    Falls back to population if nothing is selected yet.
    """
    # 1. Decide which column to colour by
    colour_col = None
    if selected_subcat:
        pct = f"{selected_subcat}_pct"
        colour_col = pct if pct in state_df_pct.columns else selected_subcat
    elif selected_category:
        first = ATTRIBUTE_MAP.get(selected_category, [None])[0]
        if first:
            pct = f"{first}_pct"
            colour_col = pct if pct in state_df_pct.columns else first

    # 2. Build lists for choropleth
    locations, values = [], []
    for feat in india_geo["features"]:
        ...
        if colour_col:
            v = state_value(state_name, colour_col)
            values.append(v if pd.notna(v) else 0)
        else:
            key = state_name.lower().replace(" ", "_").replace("-", "_")
            values.append(population_data.get(key, 0))

    legend_title = "Population" if not colour_col else short_label(colour_col)

    fig = px.choropleth_mapbox(
        geojson=india_geo,
        locations=locations,
        featureidkey="properties.st_nm"
            if "st_nm" in india_geo["features"][0]["properties"]
    ...
    # 3. Thick outline for the selected state
    if selected_state:
        for feature in india_geo["features"]:
    ...
    fig.update_layout(margin=dict(l=60, r=0, t=0, b=0))
    return fig
...
# =========================
@app.callback(
    Output("kpi-district", "children"),
    Output("kpi-state", "children"),
    Output("kpi-delta", "children"),
    Input('state-selector', 'value'),
    Input('subcategory-dropdown', 'value'),
    Input('india-map', 'clickData')            # or whatever input gives selected district
)
def update_kpis(selected_state, selected_subcat, clickData):
    # 1. If anything is missing, show placeholders
    if not selected_state or not selected_subcat or not clickData:
        return "-", "-", "-"

    # 2. Extract district name from map click
    district = clickData['points'][0]['location']  # adjust depending on your map's hovertemplate

    # 3. Load the per-district data (ensure you've already read it or reuse your df)
    df = pd.read_csv('districtwise_data_percentages.csv')
    df_state = df[df['State name'].str.lower() == district.lower()]
    # But you probably want: df_dist = df[(df['State name']==state_name)&(df['District name']==district)]

    df_dist = df[
        (df['State name'].str.lower() == selected_state.replace("_"," ").lower()) &
        (df['District name'] == district)
    ]

    # 4. Pick the correct column (pct if available)
    col = selected_subcat + '_pct' if (selected_subcat + '_pct') in df_dist.columns else selected_subcat

    # 5. Compute values
    v_dist = df_dist[col].iloc[0]
    # state average from your state_df_pct
    v_state = state_df_pct.loc[
        state_df_pct["State name"].str.lower() == selected_state.replace("_"," ").lower(),
        col
    ].iloc[0]

    # 6. Format delta and pct‐delta
    delta = v_dist - v_state
    pct = (delta / v_state * 100) if v_state else 0

    # 7. Return nicely formatted strings
    return (
        f"{v_dist:.2f}",
        f"{v_state:.2f}",
        f"{delta:+.2f} ({pct:+.1f}%)"
