...
# Map from state name to filename (handle both 'name' and 'st_nm' if one is missing)
state_file_map = {}
...
# Example population data (replace with actual values as needed)
population_data = {
...
# Short names for subcategories for bar chart axis
...
def draw_india_map(_, selected_state):
    ...
    fig = px.choropleth_mapbox(
        ...
        color=pops,
        color_continuous_scale="YlOrRd",  # Use yellow-red mix colormap
    ...
    # Remove marker_line_color and marker_line_width from update_traces, as these are not supported for choropleth_mapbox
    fig.update_traces(
        hovertemplate="%{location}<extra></extra>"
    )
    fig.update_layout(
        clickmode='event+select',
        margin={"r":0,"t":0,"l":60,"b":0},  # add left margin for colorbar
        paper_bgcolor='white',
    ...
    # Simulate pop-out effect for selected state
    if selected_state:
        for f in india_geo['features']:
            props = f.get('properties', {})
            state = props.get('st_nm') or props.get('name')
...
def update_state_district_map(selected_state, selected_subcat):
    ...
    try:
        ...
        # Draw polygons for each district
        fig = go.Figure()
        for feature in state_geo['features']:
            props = feature.get('properties', {})
