...
with open(r"C:\Users\tomar\Downloads\project661_1\project661\india.json", encoding="utf-8") as f:
...
df_pop = pd.read_csv(r'C:\Users\tomar\Downloads\project661_1\statewiseaggregated.csv')
df_pop['key'] = df_pop['State name'].str.lower().str.replace(" ", "_").str.replace("-", "_")
population_data = dict(zip(df_pop['key'], df_pop['Population']))
...
app.layout = html.Div([
    # --- Left Panel: India Map and Selectors ---
    html.Div([
        # Dropdowns panel (fixed height, always on top)
        html.Div([
            dcc.Dropdown(
                id='state-selector',
        ...
        # India map panel (fixed height, always below dropdowns)
        html.Div([
            dcc.Graph(id='india-map', style={'height': '320px', 'width': '100%', 'margin': '0', 'padding': '0'})
        ], style={
            'width': '100%',
            'backgroundColor': 'white',
            'padding': '0',
        ...
        # Bottom half: (empty or for future use)
        html.Div([], style={
            'flex': '1 1 0%',
            'width': '100%',
            'backgroundColor': 'white',
...
], style={'display': 'flex', 'flexDirection': 'row', 'width': '100vw', 'height': '100vh', 'backgroundColor': 'white', 'position': 'relative'})
...
def draw_india_map(_, selected_state):
    ...
    if selected_state:
        ...
        fig = px.choropleth_mapbox(
            ...
            zoom=2.2,  # reduced zoom for smaller map window
            opacity=1.0,
    ...
    fig.update_layout(
        ...
        coloraxis_colorbar=dict(
            ...
            xanchor='left',
            x=0.0,
            thickness=8,  # reduced from 15/10 to 8
            len=0.35      # reduced from 0.7/0.35 to 0.35 (already halved, keep as is)
        )
    )
...
def update_district_bar(selected_state, selected_category):
    ...
    if not selected_state or not selected_category:
        return go.Figure()
    df = pd.read_csv(r'C:\Users\tomar\Downloads\project661_1\statewiseaggregated.csv')
    # Normalize state names to match keys like 'uttar_pradesh'
    '''df['key'] = df['State name'].str.lower().str.replace(" ", "_").str.replace("-", "_")

     # Create dictionary: {normalized_state_name: population}
    population_data = dict(zip(df['key'], df['Population']))'''
...
def update_box_whisker_new(selected_state, selected_subcat):
    ...
    if not selected_state or not selected_subcat:
        raise PreventUpdate
    try:
        df = pd.read_csv(r'C:\Users\tomar\Downloads\project661_1\project661\districtwise_data_percentages.csv')
...
def update_state_district_map(selected_state, selected_subcat):
    ...
    try:
        ...
        # Get map center and zoom for full state view
        all_coords = []
        ...
        if all_coords:
            ...
            # Calculate zoom: rough heuristic based on bounding box size
            lat_range = max(lats) - min(lats)
            lon_range = max(lons) - min(lons)
            max_range = max(lat_range, lon_range)
            # The smaller the range, the higher the zoom. These values work well for India states.
            if max_range < 1:
                zoom = 6.8  # was 7.5
            elif max_range < 2:
                zoom = 5.8  # was 6.5
            ...
            center_lat, center_lon, zoom = 23, 80, 5
...
def update_correlation_scatter(selected_state, selected_subcat, correlation_attr_cat):
    ...
    try:
        ...
        fig = go.Figure(go.Scatter(
            x=df_state[x_col],
            y=df_state[y_col],
            mode='markers',  # Removed '+text' to hide labels
            # text=df_state['District name'],  # Removed text labels from always showing
            ...
            text=df_state['District name'],  # Only for hover
        ))
        fig.update_layout(
...
# =========================
# Synchronize Dropdown and Map Click
# =========================
...
def update_dropdown_on_map_click(clickData, current_value):
    ...
    if clickData and 'points' in clickData and len(clickData['points']) > 0:
        ...
        if state_clicked:
            key = state_clicked.lower().replace(" ", "_").replace("-", "_")
            return key
    return current_value
